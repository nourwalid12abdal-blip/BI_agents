...
Always set needs_chart to True unless the user explicitly says "do not show chart" or "no chart".
...
- SQL tables and MongoDB collections must not be mixed
...
def _validate_and_correct(result: dict, sql_tables: list, mongo_cols: list) -> dict:
    """
    Fixes a specific mistake the model makes consistently:
...
def understand_node(state: AgentState) -> AgentState:
    ...
    try:
        ...
        if not entities:
            ...
            logger.warning(
                f"[understand_node] No valid entities — falling back to: {entities}"
            )
